...
const ShoppingList = () => {
    ...
    const handleToggleChecked = async (id) => {
        ...
        const updatedItems = items.map(item =>
            item.id === id ? { ...item, is_checked: !(item.is_checked || false) } : item
        );
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            {showAddModal && (
                <AddItemModal
                    ...
                    onSuccess={() => {
                        // Reload the entire list to ensure correct data
                        fetchShoppingList();
    ...
    return (
        <div className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors group">
            <button
                ...
                className="shrink-0"
            >
                <div className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-all ${isChecked
